...
def encode_categoricals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    yes_no = [
        ...
        'HadArthritis','HadSkinCancer','DifficultyWalking','DifficultyConcentrating',
        'DifficultyDressingBathing','DifficultyErrands','FluVaxLast12','HIVTesting',
        'PneumoVaxEver','ChestScan','PhysicalActivities','DeafOrHardOfHearing',
    ...
    if 'CovidPos' in df.columns:
        df['CovidPos'] = df['CovidPos'].apply(
            lambda x: 1 if isinstance(x, str) and ('Yes' in x or 'positive' in x) else 0
        )
    ...
    age_map = {
       ...
       'Age 60 to 64':62,'Age 65 to 69':67,'Age 70 to 74':72,'Age 75 to 79':77,'Age 80 or older':85
    }
    if 'AgeCategory' in df.columns:
        df['AgeMidpoint'] = df['AgeCategory'].map(age_map).fillna(0)
        df.drop('AgeCategory', axis=1, inplace=True)

    # HadDiabetes multi-level
    diabetes_map = {
        'No':0,'No, pre-diabetes or borderline diabetes':1,'Yes':2,
...
def build_preprocessor() -> ColumnTransformer:
    numeric_feats = [
        ...
        'HeightInMeters','WeightInKilograms','AgeMidpoint',
        'Sex','CovidPos','HadAsthma','HadCOPD','HadDepressiveDisorder',
        'HadKidneyDisease','HadArthritis','HadSkinCancer','DifficultyWalking',
...
def build_filter_selector(k: int = 50) -> Pipeline:
    # Filter-based feature selector: variance threshold then mutual info K-best
    return Pipeline([
        ('var', VarianceThreshold(threshold=0.01)),
        ('mi', SelectKBest(
            score_func=lambda X, y: mutual_info_classif(X, y, discrete_features='auto'),
            k=k
        ))
    ])
...
def prepare_data(path: str,
    ...
    print('DEBUG non-numeric after encode:', nonnum)
